...
const cooldownMessages = [
  "Whoa there, speed racer! 🏎️ Let the wisdom sink in first.",
...
const Index = () => {
  ...
  const handleNewQuote = useCallback(() => {
    // If on cooldown, show another message and extend cooldown
    if (isOnCooldown) {
    ...
    const now = Date.now();
    // Track click even if loading (to detect spam attempts)
    clickTimestamps.current = [...clickTimestamps.current.filter(t => now - t < 2500), now];

    // If more than 3 clicks in 2.5 seconds, trigger cooldown
    if (clickTimestamps.current.length > 3) {
      const randomMessage = cooldownMessages[Math.floor(Math.random() * cooldownMessages.length)];
      setCooldownMessage(randomMessage);
      setIsOnCooldown(true);
      clickTimestamps.current = []; // Reset click history

      // Auto-hide message and end cooldown after 3 seconds
      cooldownTimeoutRef.current = setTimeout(() => {
        setCooldownMessage(null);
        setIsOnCooldown(false);
      }, 3000);
    ...
    // Don't trigger new quote if already loading
    if (isLoading) return;
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* Share button - fixed bottom right, only shown after first quote */}
      {hasGeneratedQuote && (
